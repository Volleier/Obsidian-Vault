**数据链路层**（Data Link Layer）是[[HAACA-OSI七层模型|OSI模型]]第二层，位于[[AADAE-Physical Layer 物理层|物理层]]与[[AADAC-Network Layer 网络层|网络层]]间。在广播式多路访问链路中（局域网），由于可能存在介质争用，它还可以细分成[介质访问控制]（MAC）子层和[逻辑链路控制]（LLC）子层，介质访问控制（MAC）子层专职处理介质访问的争用与冲突问题。
