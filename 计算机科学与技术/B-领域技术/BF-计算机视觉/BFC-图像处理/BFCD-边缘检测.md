...
**常见实现流程（Marr–Hildreth 风格）**

1. 选择尺度 σσ（与噪声/边缘尺度相关）。
2. 用高斯 G(σ)G(σ) 平滑图像： Is=I∗GIs​=I∗G。
3. 计算 LoG（或对平滑图像做离散 Laplacian）： L=∇2IsL=∇2Is​。
4. 检测 LL 的零交叉点（并结合幅值阈值以去除微小震荡）。
5. 可选后处理：非极大值抑制、形态学清理、连通性过滤。

**DoG 近似**  
差分高斯（DoG）是 LoG 的近似：用两种不同 σ 的高斯相减，计算更快且常用于尺度空间近似。

#### 优缺点

- 优点：各向同性（对方向无偏），能检测“零交叉”型边缘与曲率信息，适合多尺度分析。
- 缺点：二阶导对高频噪声放大，必须平滑；不能直接给出边缘方向；零交叉检测对噪声与量化敏感，需要阈值/后处理。
