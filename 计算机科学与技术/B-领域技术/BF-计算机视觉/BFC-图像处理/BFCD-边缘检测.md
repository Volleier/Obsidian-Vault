...
- 高质量边缘能够显著简化后续几何/形状分析
- 在 MATLAB 中可使用 `edge(image, 'canny')` 直接调用 Canny

---

## 参见

- [[BFCA-卷积与滤波]]：卷积定义、卷积与滤波关系（以及“先平滑再求导”的卷积视角）
- [[BFCC-去噪与增强]]：噪声类型、平滑滤波、锐化
- [[BFCF-尺度空间与图像金字塔]]：高斯尺度空间（$\sigma$ 与尺度）
- [[BFCB-频域分析（傅里叶）]]：高通/低通与频域理解