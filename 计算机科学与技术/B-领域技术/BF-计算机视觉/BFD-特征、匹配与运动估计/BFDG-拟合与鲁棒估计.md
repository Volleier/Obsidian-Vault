...
## 2. 总体最小二乘 / 总体最小二乘法（Total Least Squares, TLS）与几何视角
...
# 算法
## 4. RANSAC：随机采样一致性（Random Sample Consensus）
...
- 对于图像配准/单应性估计：使用归一化 DLT 在最小样本上估计模型，RANSAC 用重投影误差作为内点判定标准，最后在内点上用加权最小二乘精修并迭代剔除离群点。
- 选阈值时可先估计检测/匹配的不确定度（如角点亚像素、描述子匹配误差）并乘以安全系数。
- 在实现上使用浮点计算并避免直接比较浮点为零，使用稳健的 SVD/QR 库函数。
- 若内点比率极低，考虑使用 PROSAC 或引入先验（如匹配置信度）引导采样。

---

## 参考与延伸阅读

- Hartley, R. & Zisserman, A., Multiple View Geometry in Computer Vision.（第 7 章-第 11 章涉及拟合与鲁棒估计/ RANSAC）
