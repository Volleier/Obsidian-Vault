...
![[Player玩家-1.png]]
如上图，Player和一个PlayerController关联起来，因此虚幻引擎就可以把输入和PlayerController关联起来，这也符合了[[CBF-PlayerController玩家角色控制器|PlayerController]]接受玩家输入的描述。不管是本地玩家还是远程玩家，都需要PlayerController控制一个玩家Pawn的，所以自然也就需要为每个玩家分配一个PlayerController，所以把PlayerController放在UPlayer基类里是合理的。
