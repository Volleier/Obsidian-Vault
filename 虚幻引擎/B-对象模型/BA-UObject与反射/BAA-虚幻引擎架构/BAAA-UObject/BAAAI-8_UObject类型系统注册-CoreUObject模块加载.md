...
在前面的文章里，讲解了在static阶段的信息收集。在[[BAAAF-5_UObject类型系统信息收集]]中最后UObject的收集有提及`IMPLEMENT_VM_FUNCTION(EX_CallMath, execCallMathFunction)`会触发`UObject::StaticClass()`的调用，因此作为最开始的调用，会生成第一个UClass*
```cpp
