...
接收端验证：用 `1101001` 除以 `1011`，余数为0则无误。
#### 常见生成多项式
...
若原始数据位数为 kk，校验位数为 rr，需满足：

2r≥k+r+12r≥k+r+1

例如：

- 对4位数据（如 `1011`），需3位校验位（23=8≥4+3+123=8≥4+3+1）。
    
- 对11位数据，需4位校验位（24=16≥11+4+124=16≥11+4+1）。
    

### **（2）插入校验位**

校验位的位置为 2i−12i−1（即第1, 2, 4, 8…位），数据位填充其余位置。  
...
| 方法       | 功能       | 冗余度               | 适用场景      |
| -------- | -------- | ----------------- | --------- |
| **奇偶校验** | 仅检测单比特错误 | 1位                | 低速通信      |
| **CRC**  | 检测多比特错误  | 16/32位            | 网络协议、存储   |
| **海明码**  | 纠正单比特错误  | log⁡2(k)log2​(k)位 | 内存、高可靠性系统 |
# 流量控制与可靠传输
